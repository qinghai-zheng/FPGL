...
function [SS,U,V,term2] = updatingC(A, G, c, sum_weights, NITER, islocal)
...
for iter = 1:NITER
    U1 = D1*U;
    V1 = D2*V;
    dist = L2_distance_construct(U1',V1'); 
    S = zeros(n,m);
    for i=1:n
        idxa0 = idxa{i};
        ai = A(i,idxa0);
        di = dist(i,idxa0);
        ad = (ai-0.5*lambda*di)/sum_weights; 
        S(i,idxa0) = EProjSimplex_for_FPGL(ad);
    end
    ...
    for i=1:m
        ...
        di = dist(idxa0,i);
        ad = (ai-0.5*lambda*di)/sum_weights; 
        Sm(i,idxa0) = EProjSimplex_for_FPGL(ad);
    end
